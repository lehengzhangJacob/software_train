...
// live service (form A: dev server via `adb reverse`, loopback device side;
// form B: deployed URL later).
...
const config: CapacitorConfig = {
  ...
  server: {
    // Device-side loopback; the host dev server is reached through
    // `adb reverse tcp:3000 tcp:<host-port>` (emulator and USB device alike).
