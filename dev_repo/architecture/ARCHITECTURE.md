...
### Agent 餐食记录动作（C-44 / ADR-0023）

教练可以在用户明确要求“记录、补记或录入”一餐时帮助写入一条既有
`MealRecord`。食物被提及、询问晚餐建议或普通营养复盘不会获得写入工具。
动作回合必须按 `validate_meal_record -> save_meal_record -> verify_meal_record`
执行，复用 `/api/meals` 的营养值、日期、餐别和文本边界；服务端会话提供
`userId`，模型不能选择账户。缺少关键值时只澄清或返回待确认草稿，不创建行。

保存后的记录必须由当前账户回读确认，Trace 才能显示成功动作，assistant
消息才会附带餐食记录回执。MealRecord 的字段、关系、所有权和历史数据不变，
不新增 schema、迁移或回填；餐食页手动录入和照片审核仍是同一数据真相。
