...
- Current contract: `C-43-Release - FoodMoment v0.1.8 发布` [in_progress]
- Current slice: `C-43-Release-S1`；承接 C-42 的键盘/视口适配、头像菜单与手动录餐统一、跨页教练预填；Issues #3/#4/#5/#9/#10 仍保持开放等待外部复测
    ...
    └── C-37-Release-S1 推送 main、云端部署与 v0.1.6 GitHub Release [completed：deploy 20260824210817；APK 与四张 Trace 截图]
        ...
        └── C-42 - 移动端键盘适配、头像弹层、手动录餐与跨页教练上下文 [completed：键盘、统一表面、跨页预填均已 Browser 验收]
            ...
            └── C-42-S3 跨页进入教练时预填可编辑上下文提示 [completed：2/2 合同、运动计划入口桌面/375px Browser 与截图证据]
                └── C-43-Release - FoodMoment v0.1.8 发布 [in_progress：版本同步、发布门禁、云端与 GitHub Release]
                    └── C-43-Release-S1 版本同步、云端部署与 GitHub Release [in_progress]
